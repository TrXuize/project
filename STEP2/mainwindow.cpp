...
void MainWindow::on_csv_browse_clicked()
{
    QFileDialog myFileDialog (this);

    QString fileName = myFileDialog.getOpenFileName(this,"Select csv","/home","csv(*.csv)");
    fileName = QDir::toNativeSeparators((fileName));

    if(fileName != NULL) {
        ui->csv_path->setText(fileName);
    }
}
...
void MainWindow::on_pushButton_clicked()
{
     ...
     PyObject* pModule = PyImport_ImportModule("automatic_Label");
     if (! pModule){
        qDebug()<<QObject::tr("Can't open python file\n");
     }

     QString pic_path = ui->P_path->text();
     QString csv_path = ui->csv_path->text();
     QDir path;
     ...
     if(ui->rotate->isChecked()){
         ...
         PyTuple_SetItem(args,1,Py_BuildValue("s",csv_path.toStdString().c_str()));
